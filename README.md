...
| `denorm_full` — без раскладки | 93.16% | **96.51%** | 98.88% | 1.92% |
| `denorm_mixed` — как пишут | 92.59% | **95.87%** | 98.97% | 2.08% |
| `mixed` — опечатки поверх | 87.83% | **90.23%** | 95.76% | 1.81% |
| `typo` — опечатки | **76.26%** | 76.00% | 87.43% | 1.78% |
| `real` — ошибки из корпуса | **67.36%** | 66.94% | 65.35% | 2.35% |

Тэггер добавляет 3.5 пункта на деноминализации и **не стоит ничего** в ложных
срабатываниях. На задачах опечаток он корректно бездействует — там ошибка не в
спецбуквах.
...
R   правила           гармония гласных и позиционные ограничения из грамматики;
                      работают поверх словаря, где статистики не хватает
```
...
**Книжная грамматика добавляет десятые доли.** Гармония гласных отличает
испорченное слово от правильного в 33% случаев на деноминализации — но из 850
таких слов лексикон и без правила не принимает все 850. Корпус на 40 млн
токенов уже содержит гармонию внутри себя. Правило окупается только на редких
формах хвоста, где статистики мало: +1.79 пункта на задаче `real`. Замер целиком
— [E15](docs/experiments.md).
